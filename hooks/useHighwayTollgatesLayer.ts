...
const LAYER_ID = 'infra-highway-tollgates';

interface HighwayTollgatesAPIResponse {
...
async function fetchHighwayTollgates(
  maxCount: number,
  bboxParam: string | null
): Promise<HighwayTollgatesAPIResponse> {
  const params = new URLSearchParams({ max: String(maxCount) });
  if (bboxParam) params.set('bbox', bboxParam);

  const response = await fetch(`/api/infra/highway-tollgates?${params.toString()}`, {
    cache: 'no-store',
  });
  if (!response.ok) {
    throw new Error(`Failed to load highway tollgates layer: ${response.status}`);
  }
  return (await response.json()) as HighwayTollgatesAPIResponse;
}

async function fetchHighwayTollgatesSourceProbe(): Promise<HighwayTollgatesAPIResponse> {
  const response = await fetch('/api/infra/highway-tollgates?sourceOnly=1', {
    cache: 'no-store',
  });
  if (!response.ok) {
    throw new Error(`Failed to probe highway tollgates source: ${response.status}`);
  }
  return (await response.json()) as HighwayTollgatesAPIResponse;
}
...
export function useHighwayTollgatesLayer() {
  ...
  const sourceProbeQuery = useQuery({
    queryKey: ['infra', 'highway-tollgates', 'source-probe'],
    queryFn: fetchHighwayTollgatesSourceProbe,
    staleTime: 30 * 60_000,
    retry: 1,
  ...
  const query = useQuery({
    queryKey: ['infra', 'highway-tollgates', bboxParam],
    queryFn: () => fetchHighwayTollgates(5000, bboxParam),
    staleTime: 30 * 60_000,
    retry: 2,
    placeholderData: (previousData) => previousData,
    refetchOnWindowFocus: false,
  ...
  usePolling(['infra', 'highway-tollgates', bboxParam], 30 * 60_000, visible);
  ...
  useEffect(() => {
    ...
    if (payload.warnings?.length) {
      for (const warning of payload.warnings) {
        const key = `highway-tollgates:probe:${warning}`;
        if (seenWarnings.current.has(key)) continue;
        seenWarnings.current.add(key);
        console.warn(`[Infra:highway-tollgates:probe] ${warning}`);
      }
    }
  ...
  useEffect(() => {
    ...
    if (payload.warnings?.length) {
      for (const warning of payload.warnings) {
        const key = `highway-tollgates:${warning}`;
        if (seenWarnings.current.has(key)) continue;
        seenWarnings.current.add(key);
        console.warn(`[Infra:highway-tollgates] ${warning}`);
      }
    }
