...
const LAYER_ID = 'vulnerable-child-welfare-facilities';

interface ChildWelfareAPIResponse {
...
async function fetchChildWelfare(maxCount: number, bboxParam: string | null): Promise<ChildWelfareAPIResponse> {
  const params = new URLSearchParams({ max: String(maxCount) });
  if (bboxParam) params.set('bbox', bboxParam);

  const response = await fetch(`/api/vulnerable/child-welfare-facilities?${params.toString()}`, { cache: 'no-store' });
  if (!response.ok) {
    throw new Error(`Failed to load child welfare facilities: ${response.status}`);
  }
  return (await response.json()) as ChildWelfareAPIResponse;
}
...
export function useChildWelfareFacilitiesLayer() {
  ...
  const sourceProbeQuery = useQuery({
    queryKey: ['vulnerable', 'child-welfare-facilities', 'source-probe'],
    queryFn: () => fetchChildWelfare(1, null),
    staleTime: 30 * 60_000,
    retry: 1,
  ...
  const query = useQuery({
    queryKey: ['vulnerable', 'child-welfare-facilities', bboxParam],
    queryFn: () => fetchChildWelfare(2000, bboxParam),
    staleTime: 30 * 60_000,
    retry: 2,
    placeholderData: (previousData) => previousData,
    refetchOnWindowFocus: false,
  ...
  usePolling(['vulnerable', 'child-welfare-facilities', bboxParam], 30 * 60_000, visible);
  ...
  useEffect(() => {
    ...
    if (payload.warnings?.length) {
      for (const warning of payload.warnings) {
        const key = `child-welfare:probe:${warning}`;
        if (seenWarnings.current.has(key)) continue;
        seenWarnings.current.add(key);
        console.warn(`[Child:welfare:probe] ${warning}`);
      }
    }
  ...
  useEffect(() => {
    ...
    if (payload.warnings?.length) {
      for (const warning of payload.warnings) {
        const key = `child-welfare:${warning}`;
        if (seenWarnings.current.has(key)) continue;
        seenWarnings.current.add(key);
        console.warn(`[Child:welfare] ${warning}`);
      }
    }
