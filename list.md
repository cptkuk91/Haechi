...
| `infra` | 도로공사 영업소 + 전기차 충전소 연동 완료 | 다음 체감형 인프라 레이어 검토 |
...
- 진행 상태: 완료
- 완료 메모: `infra-ev-chargers` 레이어 추가, 실데이터 marker + 상세 패널 + 로딩 토스트 연동 완료
...
| 6 | 한국환경공단 전기자동차 충전소 정보 | `전기차-충전소` | `infra` | 신규 추가 | [공공데이터포털](https://www.data.go.kr/data/15076352/openapi.do) | 완료, `infra-ev-chargers` 실데이터 marker + 상세 패널 연동 |
