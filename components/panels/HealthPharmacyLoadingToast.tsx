...
import { LoaderCircle, MapPinned, Pill } from 'lucide-react';
import { useIsFetching } from '@tanstack/react-query';
import { useAppStore } from '@/stores/app-store';
import { getHealthPharmacyFeatureLimitForZoom } from '@/lib/health-pharmacy';

export function HealthPharmacyLoadingToast() {
  const healthPharmacyFetchCount = useIsFetching({ queryKey: ['health', 'pharmacy'] });
  const layerVisible = useAppStore((s) => Boolean(s.layers['health-pharmacy-locations']?.visible));
  const zoom = useAppStore((s) => s.camera.zoom);
  const featureLimit = getHealthPharmacyFeatureLimitForZoom(zoom);

  const isVisible = layerVisible && healthPharmacyFetchCount > 0;

  return (
    <AnimatePresence>
      {isVisible ? (
        <motion.div
          key="health-pharmacy-loading"
        ...
        >
          <div className="overflow-hidden rounded-2xl border border-sky-400/25 bg-[#091018]/92 px-4 py-3 shadow-[0_18px_48px_rgba(0,0,0,0.35)] backdrop-blur-md">
            <div className="mb-2 flex items-center justify-between gap-3">
              ...
              <Pill className="h-4 w-4 text-sky-300/75" />
            </div>
