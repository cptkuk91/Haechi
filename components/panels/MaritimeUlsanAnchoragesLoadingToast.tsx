...
export function MaritimeUlsanAnchoragesLoadingToast() {
  const fetchCount = useIsFetching({ queryKey: ['maritime', 'ulsan-anchorages'] });
  const layerVisible = useAppStore((s) => Boolean(s.layers['maritime-ulsan-anchorages']?.visible));
  ...
  return (
    <AnimatePresence>
      {isVisible ? (
        <motion.div
          key="maritime-ulsan-anchorages-loading"
        ...
        >
          <div className="overflow-hidden rounded-2xl border border-emerald-400/25 bg-[#08111a]/92 px-4 py-3 shadow-[0_18px_48px_rgba(0,0,0,0.35)] backdrop-blur-md">
            <div className="mb-2 flex items-center justify-between gap-3">
              ...
              <Anchor className="h-4 w-4 text-emerald-300/75" />
            </div>

            <p className="text-[10px] leading-relaxed text-emerald-300/90">
              울산항 정박지 polygon, circle, bunker ring geometry를 동기화합니다.
            </p>
