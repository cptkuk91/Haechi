...
import {
  ...
  toCapacityFeatureProperties,
  type EmergencyCapacitySnapshot,
  ...
  for (const row of rows) {
    ...
    features.push({
      ...
      properties: {
        ...
        phone,
        source: 'nmc',
        ...toCapacityFeatureProperties(capacity),
      },
    });
  }
...
export async function GET() {
  ...
  const capacityResult = rows.length > 0
    ? await fetchEmergencyCapacityIndex({ apiKey })
