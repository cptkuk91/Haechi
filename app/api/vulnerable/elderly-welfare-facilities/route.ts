...
const VWORLD_DATASET = 'LT_P_MGPRTFB';
...
function buildFallbackFeatureId(args: {
  ...
  const normalizedName = (args.name || `elderly-welfare-${args.index + 1}`)
    .toLowerCase()
    .replace(/[^a-z0-9\uac00-\ud7a3]+/g, '-')
    .replace(/^-+|-+$/g, '');
  ...
  if (args.feature.geometry?.type === 'Point') {
    ...
    if (typeof lng === 'number' && typeof lat === 'number') {
      return `vworld-elderly-welfare-${normalizedName}-${normalizedAddress}-${lng.toFixed(5)}-${lat.toFixed(5)}`;
    }
  }

  return `vworld-elderly-welfare-${normalizedName}-${normalizedAddress}-${args.index + 1}`;
}
...
function sanitizeFeature(feature: GeoJSON.Feature, index: number): GeoJSON.Feature {
  ...
  const category = compactText(categoryRaw) || '노인복지시설';
  const name = compactText(nameRaw) || `노인복지시설-${index + 1}`;
...
async function fetchElderlyWelfarePage(args: {
  page: number;
  key: string;
  domain?: string;
  geomFilter: string;
  ...
  return fetchVWorldFeaturePage({
    warningLabel: 'VWorld elderly welfare',
...
async function collectFeatures(args: {
  ...
  for (let page = 1; page <= totalPages && page <= MAX_PAGES; page += 1) {
    const pageResult = await fetchElderlyWelfarePage({
      page,
      key: args.key,
      domain: args.domain,
      geomFilter: args.geomFilter,
    ...
    return NextResponse.json(
      {
        ...
        updatedAt: new Date().toISOString(),
        data: emptyFeatureCollection(),
        warnings: warnings.length > 0 ? warnings : ['VWorld elderly welfare returned no features'],
      },
      {
        headers: {
          'cache-control': 'no-store, max-age=0',
